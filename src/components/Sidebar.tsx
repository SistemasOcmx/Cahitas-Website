  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 bottom-0 z-[60] group transition-all duration-500 ${isCollapsed ? 'w-20' : 'w-80'} ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        {/* Background con transición de color - SIN REDONDEADO */}
        <div className={`absolute inset-0 transition-colors duration-500 shadow-2xl ${isDarkMode
        ...
        <div className="relative h-full flex flex-col py-6 px-4">
          ...
          <div className="relative flex items-center justify-between mb-6 px-2">
            ...
            <div className={`flex items-center gap-3 transition-all duration-300 ${isCollapsed ? 'flex-col' : 'flex-row'}`}>
              ...
              <div className={`rounded-2xl overflow-hidden flex items-center justify-center shadow-xl transition-all duration-300 flex-shrink-0 ${isDarkMode ? 'shadow-blue-500/50' : 'shadow-blue-500/30'
              ...
              <div className={`transition-all duration-300 overflow-hidden ${isCollapsed ? 'opacity-0 w-0 h-0' : 'opacity-100 w-auto'}`}>
                <h1 className={`text-lg font-bold transition-colors duration-300 leading-tight ${isDarkMode ? 'text-white' : 'text-gray-900'
                  }`}>Cahita</h1>
            ...
            <button
              ...
              className={`hidden lg:flex w-8 h-8 rounded-lg items-center justify-center transition-all duration-300 flex-shrink-0 ${isDarkMode
          ...
          <div className={`mb-6 transition-all duration-300 ${isCollapsed ? 'opacity-0 scale-0 h-0' : 'opacity-100 scale-100'}`}>
            <div className="relative px-2">
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl text-sm transition-all duration-300 focus:outline-none focus:ring-2 ${isDarkMode
              ...
              />
              <svg
                className={`absolute right-5 top-1/2 -translate-y-1/2 w-4 h-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'
                  }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`group/item relative w-full flex items-center rounded-xl transition-all duration-300 ${isCollapsed ? 'justify-center p-3' : 'gap-4 px-4 py-3.5'
                      } ${isActive
                        ? isDarkMode
                  ...
                  >
                    ...
                    {/* Icono */}
                    <div className={`flex-shrink-0 transition-all duration-300 ${isHovered ? 'scale-110 rotate-3' : 'scale-100'
                      } ${isActive ? 'drop-shadow-lg' : ''}`}>
                      {item.icon}
                    </div>

                    {/* Label */}
                    <span className={`font-semibold text-sm transition-all duration-300 overflow-hidden whitespace-nowrap ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'
                      }`}>
                      {item.label}
                    </span>
                    ...
                    {isCollapsed && isHovered && (
                      <div className={`absolute left-full ml-4 px-4 py-2 rounded-xl text-sm whitespace-nowrap z-50 shadow-2xl transition-colors duration-300 ${isDarkMode
                          ? 'bg-slate-800 text-white border border-slate-700'
                          : 'bg-white text-gray-900 border border-gray-200'
                        }`}>
                        {item.label}
                        <div className={`absolute right-full top-1/2 -translate-y-1/2 border-8 border-transparent ${isDarkMode ? 'border-r-slate-800' : 'border-r-white'
                          }`} />
                      </div>
                    )}
                  </button>
          ...
          <div className={`px-4 mt-6 pb-4 transition-all duration-300 ${isCollapsed ? 'opacity-0 scale-0 h-0' : 'opacity-100 scale-100'}`}>
            {/* Separador */}
            <div className={`h-px mb-6 transition-colors duration-300 ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'
              }`} />

            {/* Toggle Dark Mode */}
            <div className={`flex items-center justify-between p-4 rounded-xl transition-all duration-300 ${isDarkMode
                ? 'bg-slate-700 hover:bg-slate-600'
                : 'bg-gray-100 hover:bg-gray-200'
              }`}>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors duration-300 ${isDarkMode
                ...
                <div>
                  <p className={`text-sm font-semibold transition-colors duration-300 ${isDarkMode ? 'text-white' : 'text-gray-900'
                    }`}>
              ...
              <button
                ...
                className={`relative w-14 h-7 rounded-full transition-all duration-300 ${isDarkMode ? 'bg-blue-600' : 'bg-gray-300'
                  }`}
              >
                <div className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${isDarkMode ? 'translate-x-7' : 'translate-x-0'
                  }`} />
              </button>
            </div>
            ...
            {/* Redes sociales */}
            <div className="mt-4">
              <p className={`text-xs font-medium mb-3 transition-colors duration-300 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'
                }`}>
                Síguenos
              </p>
